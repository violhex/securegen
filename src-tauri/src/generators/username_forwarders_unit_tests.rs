...
mod addyio_unit_tests {
    ...
    async fn test_various_http_errors() {
        ...
        for (status_code, _description) in test_cases {
            ...
            match status_code {
                401 => assert!(matches!(result, Err(UsernameError::InvalidApiKey))),
                403 => assert!(matches!(result, Err(UsernameError::DomainRejected))),
                429 => assert!(matches!(result, Err(UsernameError::RateLimitExceeded))),
                _ => {
                    assert!(result.is_err());
                    // Should be an http error for other status codes
                    assert!(matches!(result, Err(UsernameError::Http(_))));
                }
            }
...
mod fastmail_unit_tests {
    ...
    async fn test_jmap_error_response() {
        ...
        assert!(result.is_err());
        if let Err(UsernameError::ResponseContent { status: _, message }) = result {
...
mod simplelogin_unit_tests {
    ...
    async fn test_url_encoding_in_hostname() {
        ...
        // Let's be more flexible with the mock to catch any request
        server
            .register(
                Mock::given(method("POST"))
                    .and(path("/api/alias/random/new"))
...
mod edge_case_tests {
    ...
    async fn test_empty_strings_handling() {
        ...
        let _result = crate::generators::username_forwarders::addyio::generate(
            &client,
            "token".to_string(),
    ...
    async fn test_unicode_characters() {
        ...
        let _result = crate::generators::username_forwarders::simplelogin::generate_with_api_url(
            &client,
            "token".to_string(),
            server.uri(),
